# Conker: Live & Reloaded — static recompilation

> Bring-up experimental de **Conker para la Xbox original**, construido sobre
> `xboxrecomp` y orientado a producir un ejecutable nativo para Windows.

![Estado](https://img.shields.io/badge/estado-en%20desarrollo-f59e0b)
![Plataforma](https://img.shields.io/badge/plataforma-Windows%20x64-2563eb)
...
| Área | Estado |
|---|---|
| Análisis del XBE de Conker | ✅ 27 secciones cargadas |
| Traducción x86 → C | ✅ Proyecto generado |
| Compilación MSVC/Ninja | ✅ Ejecutable nativo |
| Kernel y memoria Xbox | 🟡 Bring-up activo |
| D3D / NV2A | 🟡 Inicialización y push-buffer |
| Juego visible y controlable | 🔴 Todavía no |

El ejecutable alcanza el código principal de Conker, inicializa buena parte del
dispositivo gráfico y supera varios caminos que originalmente perdían la pila.
El trabajo actual está en completar la sincronización GPU y los exports de
kernel que faltan. Consulta [el espacio de trabajo de Conker](work/conker/README.md)
para compilar, ejecutar y ver la organización de las continuaciones recuperadas.

## Prueba rápida

Necesitas Windows, Visual Studio Build Tools con C/C++, CMake, Ninja y una copia
**extraída** de un disco que poseas legalmente. La carpeta debe contener
`default.xbe`; la ISO no se carga directamente.
...
- `src/`: runtime, kernel y backends gráficos de xboxrecomp.
- `tools/`: análisis, desensamblado y recompilación estática.
- `work/conker/project/`: ejecutable anfitrión específico de Conker.
- `work/conker/recomp_single_*`: bloques internos recuperados manualmente.
- `work/conker/seed_functions.json`: semillas reproducibles del análisis.

## Aviso legal

Este repositorio no incluye la ISO, `default.xbe`, assets ni partidas del juego.
Debes proporcionar tu propia copia legal y extraerla localmente. El proyecto es
experimental y no está afiliado con Microsoft, Rare ni Xbox.

---

## Documentación original de xboxrecomp
