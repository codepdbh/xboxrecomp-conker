...
volatile uint32_t g_icall_trace[256] = {0};
...
BOOL xbox_MemoryLayoutInit(const void *xbe_data, size_t xbe_size)
{
     ...
     */
    {
         ...
         * accesses a D3D cache structure. Point it at a small fake KPCR
         * in otherwise-unused low memory and leave its cache pointer null,
         * causing the optional cache callback to be skipped.
         */
        #define FAKE_KPCR_VA    0x00001000
        MEM32_INIT(0x20, FAKE_KPCR_VA);
        MEM32_INIT(FAKE_KPCR_VA + 0x250, 0x00000000);
    ...
    /*
     * Allocate a small window at the start of Xbox kernel address space.
     *
     ...
     * D3D8 also writes a cache-control value at 0x80000000, so include
     * the preceding 64-KB allocation-granularity block.
     *
     * The actual native address is 0x80000000 + g_memory_offset.
     */
    {
        #define XBOX_KERNEL_BASE 0x80000000u
        #define XBOX_FAKE_PE_BASE 0x80010000u
        #define KERNEL_PAGE_SIZE 0x20000u
        ...
        if (g_kernel_memory) {
            ...
            *(uint32_t *)((uint8_t *)g_kernel_memory +
                          (XBOX_FAKE_PE_BASE - XBOX_KERNEL_BASE) + 0x3C) = 0x80;
            fprintf(stderr,
                    "  Kernel: mapped Xbox VA 0x%08X-0x%08X; fake PE at 0x%08X\n",
                    XBOX_KERNEL_BASE, XBOX_KERNEL_BASE + KERNEL_PAGE_SIZE,
                    XBOX_FAKE_PE_BASE);
        } else {
