    ...
    PLARGE_INTEGER Timeout)
{
    ...
    /*
     * KeWaitForSingleObject receives a pointer to an Xbox dispatcher object,
     * not necessarily a Win32 HANDLE.  The NV2A/D3D runtime uses an embedded
     * dispatcher object while waiting for the push-buffer puller.  Since the
     * current NV2A hook consumes that work synchronously, acknowledge an
     * otherwise-invalid guest dispatcher object as signalled.  Without this
     * compatibility path the title spins forever on STATUS_UNSUCCESSFUL.
     */
