...
#define BRIDGE_ICALL_TRACE_SIZE 256u
extern volatile uint32_t g_icall_trace[BRIDGE_ICALL_TRACE_SIZE];
...
/* Resolve a 32-bit Xbox handle value back to a native HANDLE.
 * Kernel APIs receive HANDLE by value. The previous implementation treated
 * that value as an Xbox VA and dereferenced it, turning valid tagged handles
 * such as 0x48000001 into unrelated guest-memory contents. */
    ...
    ULONG disposition, ULONG options)
{
    ...
    if (!name.Buffer) {
        fprintf(stderr, "  [KERNEL] file open: invalid OBJECT_ATTRIBUTES at 0x%08X\n",
                obj_attrs_va);
        bridge_write_iostatus(iostatus_va, STATUS_OBJECT_PATH_NOT_FOUND, 0);
        return STATUS_OBJECT_PATH_NOT_FOUND;
    }
    fprintf(stderr, "  [KERNEL] file open: path='%.*s' access=0x%08X "
                    "disposition=%lu options=0x%08lX\n",
            (int)name.Length, name.Buffer, (unsigned)access,
            (unsigned long)disposition, (unsigned long)options);
    ...
    fprintf(stderr, "  [KERNEL] file open: status=0x%08X handle=%p information=%llu\n",
            (unsigned)st, h, (unsigned long long)ios.Information);
    return st;
}
...
static void bridge_NtQueryVolumeInformationFile(void)
{
    ...
    memset(&ios, 0, sizeof(ios));
    fprintf(stderr, "  [KERNEL] NtQueryVolumeInformationFile: handle=%p "
                    "class=%u length=%u output=0x%08X\n",
            handle, (unsigned)infoclass, (unsigned)length, info_va);
    g_eax = (uint32_t)xbox_NtQueryVolumeInformationFile(handle, &ios,
                XBOX_TO_NATIVE(info_va), length,
                (XBOX_FS_INFORMATION_CLASS)infoclass);
    bridge_write_iostatus(ios_va, ios.Status, (uint32_t)ios.Information);
    fprintf(stderr, "  [KERNEL] NtQueryVolumeInformationFile: status=0x%08X information=%llu\n",
            (unsigned)g_eax, (unsigned long long)ios.Information);
}
...
static void kernel_thunk_dispatch(void)
{
    ...
    if (ordinal == 49) {
        uint32_t count = g_icall_trace_idx < BRIDGE_ICALL_TRACE_SIZE
            ? g_icall_trace_idx : BRIDGE_ICALL_TRACE_SIZE;
        uint32_t start = g_icall_trace_idx - count;
        uint32_t i;
        fprintf(stderr, "  [KERNEL] HalReturnToFirmware indirect-call trace:");
        for (i = 0; i < count; ++i)
            fprintf(stderr, " %08X",
                    g_icall_trace[(start + i) & (BRIDGE_ICALL_TRACE_SIZE - 1)]);
        fprintf(stderr, "\n");
    }
