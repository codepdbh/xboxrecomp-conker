...
# Derived from the export table in xboxkrnl.exe (ordinal -> RVA) joined
# with the symbol names in xboxkrnl.pdb (name -> RVA), cross-checked
# between kernel builds 3944 and 4039, which agree on every entry.
# The kernel exports by ordinal only -- its export directory carries no
# name table -- so these cannot be read off the binary alone.
# Ordinals 361-364 keep their conventional Rtl* names; the kernel
# implements them as the CRT sprintf/vsprintf and the PDB says so.
# 367-373 are null in the export table; 371 ordinals are exported.
KERNEL_EXPORTS = {
